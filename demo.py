...
paddle.set_default_dtype(d=default_float)  # 使用双精度，测试更准确
...
# rot = o3.rand_matrix()
# pos_rot = paddle.matmul(x=pos, y=rot).astype(dtype='float64')
...
# # 旋转后的pos应该旋转
# print(paddle.max(x=paddle.abs(x=paddle.matmul(x=_pos, y=rot).astype(dtype='float64') - _pos_rot)))
...
#     [1, 0, 0],
#     [0, 1, 0]
# ]).astype(dtype='float64')  # 方便起见，我们这里把H-O-H的角度设为90度
...
# rot = o3.rand_matrix()
# pos_react_rot = paddle.matmul(x=pos_react, y=rot).astype(dtype='float64')
...
xh_fixed = [
    paddle.concat(
        x=[repre[feature_type].cast("float32") for feature_type in FEATURE_MAPPING],
        axis=1,
    )
