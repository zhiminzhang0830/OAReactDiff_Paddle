...
def xh2pmg(xh):
    mol = Molecule(species=xh[:, -1].astype(dtype='int64').cpu().numpy(),
...
def pymatgen_rmsd(mol1, mol2, ignore_chirality=False, threshold=0.5,
    same_order=False):
...
def batch_rmsd(fragments_nodes: List[paddle.Tensor], out_samples: List[
    paddle.Tensor], xh: List[paddle.Tensor], idx: int=1, threshold=0.5):
    rmsds = []
    out_samples_use = out_samples[idx]
    xh_use = xh[idx]
    nodes = fragments_nodes[idx].astype(dtype='int64').cpu().numpy()
    ...
    for jj, natoms in enumerate(nodes):
        ...
        try:
            rmsd = pymatgen_rmsd(mol1, mol2, ignore_chirality=True,
                threshold=threshold)
        except:
            rmsd = 1.0
