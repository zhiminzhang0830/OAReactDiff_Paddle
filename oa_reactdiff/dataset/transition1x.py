...
class ProcessedTS1x(BaseDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if confidence_model:
            self.data["target"] = paddle.to_tensor(
                data=self.raw_dataset["target"] * repeat
            ).unsqueeze(axis=1)
            self.data["rmsd"] = paddle.to_tensor(
                data=self.raw_dataset["rmsd"] * repeat
            ).unsqueeze(axis=1)
        if ediff is not None:
            self.data["ediff"] = paddle.to_tensor(
                data=self.raw_dataset[ediff]["ediff"] * repeat
            ).unsqueeze(axis=1)
        if not only_ts:
            if not append_frag:
