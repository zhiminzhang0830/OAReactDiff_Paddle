...
class BaseDynamics(paddle.nn.Layer):
    ...
    def __init__(
        ...
        if source is not None:
            self.model.set_state_dict(state_dict=source["model"])
    ...
    def build_encoders_decoders(
        self, enfoce_name_encoding: Optional[List] = None, source: Optional[Dict] = None
    ):
        """Build encoders and decoders for nodes and edges."""
        self.encoders = paddle.nn.LayerList()
        self.decoders = paddle.nn.LayerList()
        ...
        if source is not None:
            self.encoders.set_state_dict(state_dict=source["encoders"])
            self.decoders.set_state_dict(state_dict=source["decoders"])
