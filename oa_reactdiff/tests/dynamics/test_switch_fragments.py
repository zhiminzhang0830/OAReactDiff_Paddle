...
model_config = dict(in_node_nf=8, in_edge_nf=0, hidden_nf=64,
    ...
    inv_sublayers=2, sin_embedding=False, normalization_factor=1.0,
...
class TestModel(unittest.TestCase):
    ...
    def setUpClass(cls) ->None:
        cls.egnn_dynamics = EGNNDynamics(model_config=model_config,
        ...
        cls.n_samples = 1
        cls.fragments_nodes = [paddle.to_tensor(data=[4]), paddle.to_tensor
            (data=[5])]
    ...
    def test_switch_fragments(self):
        ...
        for dynamics in self.dynamics:
            _xh, _edge_attr = dynamics.forward(self.xh, self.edge_index,
                ...
                combined_mask, edge_attr=None)
            xh = [self.xh[1], self.xh[0]]
            fragments_nodes = [self.fragments_nodes[1], self.fragments_nodes[0]
                ]
            fragments_masks = [get_mask_for_frag(natm_nodes) for natm_nodes in
                fragments_nodes]
            ...
            _xh_switch, _edge_attr_switch = dynamics.forward(xh, edge_index,
                self.t, self.conditions, n_frag_switch, combined_mask,
                edge_attr=None)
            print(_xh[0])
            print(_xh_switch[1])
            self.assertFalse(paddle.allclose(x=_xh[0], y=_xh_switch[1],
                rtol=1e-06).item())
    ...
    def test_switch_fragments_same_encoding(self):
        ...
        for dynamics in self.dynamics:
            ...
            print(_xh[0])
            print(_xh_switch[1])
            self.assertTrue(paddle.allclose(x=_xh[0], y=_xh_switch[1], rtol
                =1e-06).item())
