...
__ATOM_LIST__ = ['h', 'he', 'li', 'be', 'b', 'c', 'n', 'o', 'f', 'ne', 'na',
...
atomic_valence[9] = [1]
atomic_valence[14] = [4]
atomic_valence[15] = [5, 3]
atomic_valence[16] = [6, 3, 2]
atomic_valence[17] = [1]
atomic_valence[32] = [4]
atomic_valence[35] = [1]
atomic_valence[53] = [1]
...
def charge_is_OK(BO, AC, charge, DU, atomic_valence_electrons, atoms,
...
def BO_is_OK(BO, AC, charge, DU, atomic_valence_electrons, atoms, valences,
    ...
    check_charge = charge_is_OK(BO, AC, charge, DU,
        atomic_valence_electrons, atoms, valences, allow_charged_fragments)
...
def clean_charges(mol):
    ...
    rxn_smarts = [
        '[#6,#7:1]1=[#6,#7:2][#6,#7:3]=[#6,#7:4][CX3-,NX3-:5][#6,#7:6]1=[#6,#7:7]>>[#6,#7:1]1=[#6,#7:2][#6,#7:3]=[#6,#7:4][-0,-0:5]=[#6,#7:6]1[#6-,#7-:7]'
        ,
        '[#6,#7:1]1=[#6,#7:2][#6,#7:3](=[#6,#7:4])[#6,#7:5]=[#6,#7:6][CX3-,NX3-:7]1>>[#6,#7:1]1=[#6,#7:2][#6,#7:3]([#6-,#7-:4])=[#6,#7:5][#6,#7:6]=[-0,-0:7]1'
        ]
...
def BO2mol(mol, BO_matrix, atoms, atomic_valence_electrons, mol_charge,
    allow_charged_fragments=True, use_atom_maps=False):
    """
    ...
    if l != l2:
        raise RuntimeError('sizes of adjMat ({0:d}) and Atoms {1:d} differ'
            .format(l, l2))
    rwMol = Chem.RWMol(mol)
    bondTypeDict = {(1): Chem.BondType.SINGLE, (2): Chem.BondType.DOUBLE, (
...
def AC2BO(AC, atoms, charge, allow_charged_fragments=True, use_graph=True):
    ...
    for i, (atomicNum, valence) in enumerate(zip(atoms, AC_valence)):
        possible_valence = [x for x in atomic_valence[atomicNum] if x >=
            valence]
        if not possible_valence:
            print('Valence of atom', i, 'is', valence,
                'which bigger than allowed max', max(atomic_valence[
                atomicNum]), '. Stopping')
            sys.exit()
    ...
    for valences in valences_list:
        ...
        for UA_pairs in UA_pairs_list:
            BO = get_BO(AC, UA, DU_from_AC, valences, UA_pairs, use_graph=
                use_graph)
            status = BO_is_OK(BO, AC, charge, DU_from_AC,
                atomic_valence_electrons, atoms, valences,
                allow_charged_fragments=allow_charged_fragments)
            charge_OK = charge_is_OK(BO, AC, charge, DU_from_AC,
                atomic_valence_electrons, atoms, valences,
                allow_charged_fragments=allow_charged_fragments)
            if status:
                return BO, atomic_valence_electrons
            elif BO.sum() >= best_BO.sum() and valences_not_too_large(BO,
                valences) and charge_OK:
                best_BO = BO.copy()
    return best_BO, atomic_valence_electrons


def AC2mol(mol, AC, atoms, charge, allow_charged_fragments=True, use_graph=
    True, use_atom_maps=False):
    """ """
    BO, atomic_valence_electrons = AC2BO(AC, atoms, charge,
        allow_charged_fragments=allow_charged_fragments, use_graph=use_graph)
    mol = BO2mol(mol, BO, atoms, atomic_valence_electrons, charge,
        allow_charged_fragments=allow_charged_fragments, use_atom_maps=
        use_atom_maps)
...
def xyz2mol(atoms, coordinates, charge=0, allow_charged_fragments=True,
    use_graph=True, use_huckel=False, embed_chiral=True, use_atom_maps=False):
    """
    ...
    new_mols = AC2mol(mol, AC, atoms, charge, allow_charged_fragments=
        allow_charged_fragments, use_graph=use_graph, use_atom_maps=
        use_atom_maps)
    if embed_chiral:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--sdf', action='store_true', help=
        'Dump sdf file')
    parser.add_argument('--ignore-chiral', action='store_true', help=
        'Ignore chiral centers')
    parser.add_argument('--no-charged-fragments', action='store_true', help
        ='Allow radicals to be made')
    parser.add_argument('--no-graph', action='store_true', help=
        'Run xyz2mol without networkx dependencies')
    parser.add_argument('--use-huckel', action='store_true', help=
        'Use Huckel method for atom connectivity')
    parser.add_argument('-o', '--output-format', action='store', type=str,
        help='Output format [smiles,sdf] (default=sdf)')
    parser.add_argument('-c', '--charge', action='store', metavar='int',
        type=int, help='Total charge of the system')
    ...
    mols = xyz2mol(atoms, xyz_coordinates, charge=charge, use_graph=quick,
        allow_charged_fragments=charged_fragments, embed_chiral=
        embed_chiral, use_huckel=use_huckel)
    for mol in mols:
